...
export default function SitNumberActions({
  row,
  onEdit,
  onDelete,
  onPrint,
  ...
  const toggleDropdown = () => {
    if (ref.current) {
      ...
      const dropdownHeight = canEdit ? 140 : 70; // height depends on number of options
      ...
      if (isSingleRow) setPositionTop(true);
      else if (isFirstRow) setPositionTop(spaceBelow >= 30);
      else if (isLastRow) setPositionTop(false);
      else setPositionTop(spaceBelow >= dropdownHeight);
    }
    setOpen(!open);
  };
  ...
  return (
    <div className="relative h-6 flex items-center" ref={ref}>
      ...
      {open && (
        <div
          className={`absolute -right-4 w-28 border shadow-sm z-50 ${
        ...
        >
          ...
          <button
            ...
            className={`w-full flex items-center gap-2 px-3 py-1 text-xs border-t ${borderClr} ${
              darkMode
                ? "hover:bg-gray-700 text-gray-100"
                : "hover:bg-gray-100 text-gray-900"
            }`}
          >
            <FiPrinter
              className={`w-3 h-3 ${darkMode ? "text-green-400" : "text-green-600"}`}
            />
            Print
          </button>

          {/* Edit / Delete - only if canEdit */}
          {canEdit && (
            <>
              <button
                onClick={() => {
