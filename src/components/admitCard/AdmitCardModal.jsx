...
export default function AdmitCardModal({
  ...
  // Auto-generate admit card numbers for live view
  useEffect(() => {
  ...
  return (
    ...
    >
      <div
        ...
        className={`${modalBg} ${textColor} w-64 max-w-md border ${borderClr} p-6 max-h-[90vh] overflow-y-auto transition-all duration-300 transform ${
      ...
      >
        ...
        {allSelected && (
          <div
            className={`border ${borderClr} py-[6px] px-2 mt-2 ${darkMode ? "bg-gray-700" : "bg-gray-50"}`}
          >
        ...
        {allSelected && Object.keys(localGeneratedAdmitCards).length > 0 && (
          <div className="mt-4">
            <div
              className={`border ${borderClr} max-h-50 overflow-y-auto ${darkMode ? "bg-gray-700" : "bg-gray-50"}`}
            >
              <table className="w-full text-[12px]">
                <thead className="sticky top-0 z-10">
                  <tr
                    className={`border-b ${borderClr} ${darkMode ? "bg-gray-800" : "bg-gray-100"}`}
                  >
                    <th
                      className={`px-2 py-[6px] text-left font-semibold border-r ${borderClr} ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                    >
                      Student name
                    </th>
                    <th
                      className={`px-2 py-[6px] text-right font-semibold ${darkMode ? "text-gray-300" : "text-gray-700"}`}
                    >
                      Admit card no
                    </th>
                  </tr>
                ...
                <tbody>
                  {filteredStudents.map((student) => {
                    const examNameMap = { Mid: "Mid Term", Final: "Final" };
                    const examName =
                      examNameMap[selections.exam] || selections.exam;
                    const key = `${student.IDNumber}_${student.Class}_${student.Section || ""}_${student.Session}_${examName}`;
                    ...
                    return (
                      ...
                      >
                        <td
                          className={`px-2 py-[6px] border-r ${borderClr} ${darkMode ? "text-gray-300" : "text-gray-500"}`}
                        >
                          {student.StudentName}
                        </td>
                        <td
                          className={`px-2 py-[6px] text-right font-semibold text-blue-600 ${
