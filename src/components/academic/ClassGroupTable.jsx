...
export default function ClassGroupTable({ data, month, groupFilter }) {
    ...
    <div className="rounded overflow-x-auto">
      <table className="w-full table-auto border-collapse text-sm">
        <thead className={`${darkMode ? "bg-gray-800 border-b border-gray-700" : "bg-gray-100 border-b border-gray-200"}`}>
          <tr>
            {["SL", "Class", "Group", "Subjects No", "Student No", "Total Payable", "Payable Due"].map(h => (
          ...
          {data.length === 0 ? (
            ...
            data.map(c =>
              c.groups
                ...
                .map((g, idx) => {
                  const monthData = month === "All" ? { paid: g.monthly.reduce((a,b) => a + b.paid, 0), due: g.monthly.reduce((a,b) => a + b.due, 0) } :
                  ...
                  return (
                    <tr key={`${c.sl}-${g.name}`} className={`border-b ${borderCol} ${hoverRow}`}>
                      ...
                      <td className={`px-3 py-2 border-r ${borderCol} whitespace-nowrap`}>{g.subjects}</td>
